...
Team is 6, not 5 — roles are assigned by current skill fit rather than the
original 5-role template. Full rationale in
[WORK_DISTRIBUTION.md](WORK_DISTRIBUTION.md).
...
> Real GitHub branches already exist for the team: `Abhinavpreet`, `Meven`,
> `Noore`, `Raghavendra`, `Shifana`, `Surya`. Fill in the `_TBD_` cells once
> each remaining branch is matched to a role.
...
RocksDB / state-recovery work is folded into Stream Processing (Week 3) rather
than kept as a standalone role — see
[WORK_DISTRIBUTION.md](WORK_DISTRIBUTION.md) for the full rationale.
...
Run with `python scripts/chaos_recovery_demo.py 20 5` (requires the stack up
and `bash scripts/create_topics.sh` to have created `truck-state-changelog`):
...
without `checkpoint()` or `close()`, and its RocksDB directory is deleted —
the worst case, a replacement landing on a fresh node after a rebalance.
Worker B starts empty and rebuilds purely from Kafka.

Design notes that make this hold:

- **RocksDB is a local cache; the changelog is the source of truth.** A worker
  that keeps its disk reads locally; one that lost it replays. Both converge
  on identical state.
...
- **Replay is idempotent** (covered by
  `test_changelog_replay_is_idempotent`): records are last-write-wins per key,
  so at-least-once changelog delivery can't double-count.
