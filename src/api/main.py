...
def worker_logs(worker_id: str, tail: int = 50):
    """Return the last N lines of a worker log file (read-only)."""
    if worker_id not in worker_manager.workers:
        return {"success": False, "lines": [], "error": "unknown worker"}
    safe_tail = max(1, min(tail, 200))
    log_path = REPO_ROOT / "logs" / f"{worker_id}.log"
    if not log_path.exists():
        return {"success": True, "lines": [], "path": str(log_path), "message": "log file not created yet"}
    try:
        text = log_path.read_text(encoding="utf-8", errors="replace")
        lines = text.splitlines()[-safe_tail:]
        return {"success": True, "lines": lines, "path": str(log_path)}
    except Exception as e:
        logger.error("Failed to read worker log %s: %s", log_path, e)
